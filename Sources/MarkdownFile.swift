...
enum MarkdownDocumentError: Error {
    ...
    case failedToRenderMarkup(fileAtURL: URL)
}
...
public struct MarkdownFile {
    ...
    public func renderMarkup(fragment: Bool) throws -> String {
        ...
        do {
            ...
            if fragment {
                ...
                if let markup = try markupDocument.body()?.html() {
                    return markup
                }
            }
            else {
        ...
        throw MarkdownDocumentError.failedToRenderMarkup(fileAtURL: fileURL)
    }

    public func markupRepresentation(fragment: Bool) -> String? {
        return try? renderMarkup(fragment: fragment)
    }
    ...
    public var truncatedBodyContent: String? {
        guard let markupRepresentation = try? renderMarkup(fragment: true) else {
            return nil
        }
