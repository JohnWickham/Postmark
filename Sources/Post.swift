...
public class Post: Codable {
    ...
    convenience init(describing directory: URL, markdownFile: MarkdownFile?) throws {
        let parentDirectory = directory.deletingLastPathComponent()
        let filesHelper = PostFilesHelper(contentDirectoryURL: parentDirectory)
        let fileSet = try filesHelper.makePostFileSet(forPostAt: directory)
        try self.init(fileSet: fileSet, markdownFile: markdownFile)
    }

    init(fileSet: PostFileSet, markdownFile: MarkdownFile?) throws {
        let contentSourceFileURL = fileSet.sourceMarkdownFileURL
        ...
        if FileManager.default.fileExists(atPath: fileSet.staticContentFileURL.path) {
            self.hasGeneratedContent = true
        }

        self.slug = fileSet.slug
        ...
        self.publishStatus = fileSet.publishStatus
