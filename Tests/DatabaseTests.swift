...
final class DatabaseTests: XCTestCase {
    ...
    func testDeletePostsExcludingKeepsRegeneratedPostsAndRemovesStaleRows() throws {
        let currentPost = Post(slug: "current-post", title: "Current Post", topics: [], createdDate: Date(), publishStatus: .public)
        let stalePost = Post(slug: "stale-post", title: "Stale Post", topics: [], createdDate: Date(), publishStatus: .public)
        try DataStore.shared.addOrUpdate(currentPost)
        try DataStore.shared.addOrUpdate(stalePost)

        try DataStore.shared.deletePosts(excluding: [currentPost.slug])

        XCTAssertNotNil(try DataStore.shared.getPost(with: currentPost.slug))
        XCTAssertNil(try DataStore.shared.getPost(with: stalePost.slug))
    }
